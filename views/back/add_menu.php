...
<head>
    ...
    <title>Ajouter un menu - NutriWise</title>
    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            ...
            <div class="form-container" style="max-width: 980px;">
                ...
                <form action="index.php?page=admin_add_menu" method="POST">
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="goal" value="<?= htmlspecialchars($_POST['goal'] ?? '') ?>" placeholder="Ex: Perte de poids">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="calories_target" min="0" value="<?= isset($_POST['calories_target']) ? intval($_POST['calories_target']) : '' ?>" placeholder="Ex: 1800">
                        </div>
                        <div class="form-group">
                            ...
                            <select name="assigned_to" required>
                                ...
                                <?php foreach($users as $user): ?>
                                    <option value="<?= $user['id'] ?>" <?= (isset($_POST['assigned_to']) && intval($_POST['assigned_to']) === intval($user['id'])) ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        <label>Actif</label>
                        <input type="checkbox" name="is_active" value="1" <?= isset($_POST['is_active']) ? 'checked' : '' ?>> Activer ce menu
                    </div>
                                    ...
                                    <div style="display:grid; gap:10px;">
                                        <?php foreach($mealTypes as $mealKey => $mealLabel): ?>
                                            <div>
                                                ...
                                                <textarea name="meals[<?= $dayIndex ?>][<?= $mealKey ?>]" rows="2" placeholder="Description de <?= strtolower($mealLabel) ?>"><?= htmlspecialchars($_POST['meals'][$dayIndex][$mealKey] ?? '') ?></textarea>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                                </div>
                    ...
                    <div class="form-actions" style="margin-top: 30px;">
                        <button type="submit" class="btn-save">Enregistrer le menu</button>
